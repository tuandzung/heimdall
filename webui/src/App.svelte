...
<main>
  <div class="w-11/12 mx-auto mt-2 pb-4 flex flex-col grow min-h-screen">
      ...
      <div>
        {#if $userStore?.user}
          ...
          <button
            ...
            on:click={() => userStore.logout()}>Logout</button
          >
        {:else}
          <button
            class="underline hover:cursor-pointer"
            on:click={startGoogleOAuth}>Login with Google</button
          >
        {/if}
      </div>
    </div>
    ...
    <div class="mt-auto pt-4 text-center text-gray-500">
      <Fa fw icon={faGithub} />
      <a
        href="https://github.com/sap1ens/heimdall"
        target="_blank"
        class="underline hover:no-underline">sap1ens/heimdall</a
      >
